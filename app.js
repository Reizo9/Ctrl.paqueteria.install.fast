...
(async function(){
  ...
  // ¡¡¡IMPORTANTE!!! REEMPLAZA ESTO CON TUS PROPIAS CLAVES DE SUPABASE
  const SUPABASE_URL = 'https://djfgtpujvvmzoafrzmst.supabase.co';
  const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRqZmd0cHVqdnZtem9hZnJ6bXN0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjIyODAxNDQsImV4cCI6MjA3Nzg1NjE0NH0.7fttwKoA7ZioGZxESYoNRwfE0YAaACMHlKzSYXXr_oc';
  // ¡¡¡IMPORTANTE!!! REEMPLAZA ESTO CON TUS PROPIAS CLAVES DE SUPABASE

  let supabase = null;
  // ★★★ CORRECCIÓN DE LÓGICA ★★★
  // Comparamos con el texto de ejemplo ORIGINAL, no con tu URL real.
  ...
  window.addEventListener('online', () => { 
    isOnline = true; 
    // Usamos 'typeof' para evitar errores si showToast aún no está definido
  ...
  if(document.body.classList.contains('page-main')){
    ...
    recibirBtn.addEventListener('click', async ()=>{
      ...
      try{
        ...
        if(p) {
          ...
          action = 'add';
          // Creamos un ID local temporal basado en el timestamp para usar en Supabase
          // IMPORTANTE: Supabase debe tener 'id' como primary key y *no* ser autoincremental, o usar un UUID.
          // Por simplicidad, usaremos el timestamp como ID si es nuevo.
          // OJO: Si la PK de supabase es autoincremental, esto fallará.
          // Asumiremos que la PK de Supabase es la misma que la de IndexedDB.
          // DE HECHO, addItem devuelve el ID autoincrementado.
        ...
        if (isOnline && supabase) {
          ...
          // Asegurarnos que el ID local esté
          supabasePayload.id = localId;

          if (action === 'add') {
             // Usamos 'upsert' por si acaso el item se creó offline y ya existe en Supabase
    ...
    guardarFirma.addEventListener('click', async ()=>{
      ...
      if (currentBatchToDeliver.length > 0) {
        ...
        try {
          for (const p of currentBatchToDeliver) {
            ...
            // p.servicio ya debe estar definido
      ...
      // --- Lógica para Paquete Individual ---
        ...
        try {
          const guia = guiaEl.value.trim(); 
          // ¡MODIFICADO! Usa getByKey (es más rápido si ya sabemos la guía)
          const p = await getByKey('paquetes', (await getByIndex('paquetes', 'guia', guia)).id); // Un poco enredado, pero busca por guía
    ...
    deleteConfirmBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const deletePayload = { id: itemToDelete.id }; // Solo necesitamos el ID para borrar
    ...
    // AHORA SOLO RESPALDA EL SERVICIO ACTUAL
    ...
    // RESTAURAR TAMBIÉN HA SIDO MODIFICADO
    ...
    restoreBackupInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        // Borramos solo los datos del servicio actual
        ...
        // Forzamos la sincronización (push) de estos nuevos datos
        if(isOnline && supabase) {
    ...
    tablaUsuarios.addEventListener('click', (e) => {
      const act = e.target.dataset.act; const id = Number(e.target.id);
      if (act === 'delete_user') {
        ...
        deleteConfirmModal.classList.remove('hidden'); // Corregido, era .add('hidden')
      }
    });
      ...
      try {
        ...
        if (itemsToSync.length > 0) {
          ...
          for (const item of itemsToSync) {
            // Asegurarnos que el item pertenezca a este servicio
            ...
            try {
              if (item.action === 'add' || item.action === 'update') {
                // Usamos 'upsert' para manejar conflictos (ej: crear algo offline que ya existe)
              ...
              if (error) {
                ...
                // Si la operación fue exitosa, la borramos de la cola local
               ...
               // No borramos el item de la cola si falla, para reintentar luego
            }
          }
        } else {
        ...
        // Esto asegura que tenemos la versión más reciente de la nube
